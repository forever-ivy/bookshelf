...
import TutorWorkspaceStackLayout from '@/app/tutor/[profileId]/_layout';
...
jest.mock('expo-router', () => {
  ...
  const Stack = ({
    ...
    return React.createElement(View, { testID: 'tutor-workspace-stack' }, children);
  };
...
jest.mock('@/components/tutor/tutor-workspace-provider', () => {
  const MockTutorWorkspaceProvider = ({ children }: { children: React.ReactNode }) => children;
  MockTutorWorkspaceProvider.displayName = 'MockTutorWorkspaceProvider';

  return {
    TutorWorkspaceProvider: MockTutorWorkspaceProvider,
  };
});

describe('TutorWorkspaceStackLayout', () => {
  beforeEach(() => {
  ...
  it('registers the native info sheet route as a form sheet presentation', () => {
    render(<TutorWorkspaceStackLayout />);

    expect(screen.getByTestId('tutor-workspace-stack')).toBeTruthy();
