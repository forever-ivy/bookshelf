...
function OrderDetailSkeleton() {
  ...
  return (
    <View style={{ gap: theme.spacing.lg }} testID="order-detail-skeleton">
      <LoadingSkeletonCard>
        <LoadingSkeletonText lineHeight={16} widths={['62%', '38%', '82%']} />
...
export default function OrderDetailRoute() {
  ...
  const showOrderSkeleton = !orderQuery.data && !orderQuery.isError && Boolean(orderQuery.isFetching);
  const order = orderQuery.data;
  const canCancel = order ? order.status === 'active' || order.status === 'renewable' || order.status === 'dueSoon' : false;
  const canReturn = order ? order.status !== 'completed' && order.status !== 'cancelled' : false;
