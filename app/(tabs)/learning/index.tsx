...
export default function LearningIndexRoute() {
  ...
  const continueSession = React.useMemo(
    () =>
      [...(sessionsQuery.data ?? [])]
  ...
  const featuredProfiles = React.useMemo(() => {
    ...
    if (continueSession) {
      orderedIds.push(continueSession.learningProfileId);
    }
    ...
    return orderedIds
      .map((profileId) => profileById.get(profileId) ?? null)
      .filter((profile): profile is NonNullable<typeof profile> => profile !== null)
      .slice(0, 4);
  }, [continueSession, recentProfiles]);
