...
def upgrade() -> None:
    ...
    profile_columns = {column["name"] for column in sa.inspect(bind).get_columns("learning_profiles")}
    if "active_path_version_id" not in profile_columns:
        op.add_column(
            "learning_profiles",
            sa.Column("active_path_version_id", sa.Integer(), nullable=True),
        )
...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        op.execute("DROP INDEX IF EXISTS ix_learning_fragments_embedding_hnsw")
        op.execute("DROP INDEX IF EXISTS ix_learning_fragments_search_vector_trgm")

    for index_name in [
        "ix_learning_bridge_actions_status",
        "ix_learning_bridge_actions_to_session_id",
        "ix_learning_bridge_actions_from_turn_id",
        "ix_learning_bridge_actions_from_session_id",
        "ix_learning_bridge_actions_action_type",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_bridge_actions")

    for index_name in [
        "ix_learning_step_context_items_source_turn_id",
        "ix_learning_step_context_items_source_session_id",
        "ix_learning_step_context_items_step_index",
        "ix_learning_step_context_items_guide_session_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_step_context_items")

    for index_name in [
        "ix_learning_jobs_status",
        "ix_learning_jobs_job_type",
        "ix_learning_jobs_profile_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_jobs")

    for index_name in [
        "ix_learning_reports_report_type",
        "ix_learning_reports_session_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_reports")

    for index_name in [
        "ix_learning_remediation_plans_status",
        "ix_learning_remediation_plans_step_index",
        "ix_learning_remediation_plans_session_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_remediation_plans")

    for index_name in [
        "ix_learning_checkpoints_passed",
        "ix_learning_checkpoints_step_index",
        "ix_learning_checkpoints_turn_id",
        "ix_learning_checkpoints_session_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_checkpoints")

    for index_name in [
        "ix_learning_agent_runs_status",
        "ix_learning_agent_runs_agent_name",
        "ix_learning_agent_runs_turn_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_agent_runs")

    for index_name in [
        "ix_learning_turns_turn_kind",
        "ix_learning_turns_created_at",
        "ix_learning_turns_session_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_turns")

    for index_name in [
        "ix_learning_sessions_focus_step_index",
        "ix_learning_sessions_source_turn_id",
        "ix_learning_sessions_source_session_id",
        "ix_learning_sessions_session_kind",
        "ix_learning_sessions_remediation_status",
        "ix_learning_sessions_status",
        "ix_learning_sessions_learning_mode",
        "ix_learning_sessions_profile_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_sessions")

    for index_name in [
        "ix_learning_path_steps_step_type",
        "ix_learning_path_steps_step_index",
        "ix_learning_path_steps_path_version_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_path_steps")

    op.execute("DROP INDEX IF EXISTS ix_learning_profiles_active_path_version_id")
    with op.batch_alter_table("learning_profiles") as batch_op:
        try:
            batch_op.drop_constraint("fk_learning_profiles_active_path_version_id", type_="foreignkey")
        except Exception:
            pass
        try:
            batch_op.drop_column("active_path_version_id")
        except Exception:
            pass

    for index_name in [
        "ix_learning_path_versions_status",
        "ix_learning_path_versions_profile_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_path_versions")

    for index_name in [
        "ix_learning_fragments_fragment_type",
        "ix_learning_fragments_chunk_index",
        "ix_learning_fragments_asset_id",
        "ix_learning_fragments_profile_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_fragments")

    for index_name in [
        "ix_learning_source_assets_content_hash",
        "ix_learning_source_assets_parse_status",
        "ix_learning_source_assets_book_source_document_id",
        "ix_learning_source_assets_book_id",
        "ix_learning_source_assets_asset_kind",
        "ix_learning_source_assets_bundle_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_source_assets")

    for index_name in [
        "ix_learning_profiles_status",
        "ix_learning_profiles_difficulty_mode",
        "ix_learning_profiles_goal_mode",
        "ix_learning_profiles_source_bundle_id",
        "ix_learning_profiles_reader_id",
    ]:
        op.execute(f"DROP INDEX IF EXISTS {index_name}")
    op.execute("DROP TABLE IF EXISTS learning_profiles")

    op.execute("DROP INDEX IF EXISTS ix_learning_source_bundles_reader_id")
    op.execute("DROP TABLE IF EXISTS learning_source_bundles")
