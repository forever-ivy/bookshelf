...
export function getLibraryErrorMessage(error: unknown, fallback = '借阅服务暂时不可用，请稍后重试。') {
  ...
  if (error.code === 'learning_runtime_unavailable') {
    return typeof error.message === 'string' && error.message.trim().length > 0
      ? error.message
      : '导学生成链路暂时不可用，请检查 Redis、worker 和编排服务。';
  }

  if (error.code === 'learning_profile_not_ready') {
    return '导学本还没有准备好，请等生成完成后再进入工作区。';
  }

  if (error.code === 'learning_model_request_error') {
    return '模型请求错误';
  }

  if (error.code === 'learning_upload_empty') {
    return '上传内容为空，请换一份可解析的资料后再试。';
  }
