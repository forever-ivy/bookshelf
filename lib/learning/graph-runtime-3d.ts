...
type NodeVisualStateArgs = LabelVisibilityArgs & {
...
export function resolveLearningGraph3DLabelVisibility(
  ...
  if (node.type === 'Book' || node.type === 'SourceAsset') {
    return true;
  }

  if (args.generatedNodeIds.includes(node.id)) {
    return true;
  }
...
export function resolveLearningGraph3DNodeVisualState(
  node: RuntimeNodeLike,
